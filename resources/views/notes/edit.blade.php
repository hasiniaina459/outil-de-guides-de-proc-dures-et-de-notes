...
@section('content')
    ...
    <form action="{{ route('notes.update', $notes->id_note) }}" method="POST">
        ...
        <div>
            ...
            <div class="chip-grp">
                <input type="checkbox" name="categorie[]" class="chip-inp" id="info" value="information" {{ (is_array(old('categorie')) && in_array('information', old('categorie'))) ? 'checked' : '' }}>
                <label for="info" class="chip-lab"> information</label>
                <input type="checkbox" name="categorie[]" class="chip-inp" id="new" value="nouvelle" {{ (is_array(old('categorie')) && in_array('nouvelle', old('categorie'))) ? 'checked' : '' }}>
                <label for="new" class="chip-lab"> nouvelle</label>
                <input type="checkbox" name="categorie[]" class="chip-inp" id="fns" value="termine" {{ (is_array(old('categorie')) && in_array('termine', old('categorie'))) ? 'checked' : '' }}>
