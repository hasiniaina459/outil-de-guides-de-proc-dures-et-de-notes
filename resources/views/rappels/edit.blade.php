    ...
    <form action="{{ route('rappels.update', $rappels->id_rappel) }}" method="POST">
        ...
        <div>
            ...
            @foreach($individus as $individu)
                <div class="chip-grp">
                    <input type="checkbox" name="individu[]" class="chip-inp" value="{{ $individu->id_individu }}" id="individu_{{ $individu->id_individu }}"
                        {{ in_array($individu->id_individu, old('individu', $rappels->individus->pluck('id_individu')->toArray())) ? 'checked' : '' }}>
                    <label for="individu_{{ $individu->id_individu }}" class="chip-lab">{{ $individu->name }} {{ $individu->firstname}}</label>
                </div>
            @endforeach
            @error('id_individu')
            <div class="error">{{ $message }}</div>
