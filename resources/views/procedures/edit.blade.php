...
<form action="{{ route('procedures.update', $procedures->id_procedure) }}" method="POST">
    ...
    <div>
        ...
        <input type="radio" name="procedure_status" value="0" {{ old('procedure_status', (int) $procedures->procedure_status) == 0 ? 'checked' : '' }}> En cours
        <input type="radio" name="procedure_status" value="1" {{ old('procedure_status', (int) $procedures->procedure_status) == 1 ? 'checked' : '' }}> Terminé
    ...
    <div>
        ...
        <input type="checkbox" name="service[]" id="service_{{ $service->id_service }}" value="{{ $service->id_service }}"
            {{ in_array($service->id_service, old('service', $procedures->services->pluck('id_service')->toArray())) ? 'checked' : '' }}><label for="service_{{ $service->id_service }}">{{ $service->service_name }}</label>
        @endforeach
        @error('id_service')
        <div class="error">{{ $message }}</div>
        @enderror
