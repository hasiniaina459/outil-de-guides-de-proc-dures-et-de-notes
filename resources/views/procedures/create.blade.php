...
@section('content')
    ...
    <form action="{{ route('procedures.store') }}" method="POST">
        ...
        <div>
            ...
            <input type="radio" name="procedure_status" value="1" id="status_eff" {{ old('status') ? 'checked' : '1' }}>effectuer
            <input type="radio" name="procedure_status" value="0" id="status_neff" {{ old('status') ? 'checked' : '0' }}>non effectuer
        ...
        <div>
            ...
            @foreach($services as $service)
                <div class="chip-grp">
                    <input type="checkbox" name="service[]" value="{{ $service->id_service }}" class="chip-inp" id="service_{{ $service->id_service }}"
